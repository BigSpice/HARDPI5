...
class PiezoRecorder:
    ...
    def writer_task(self):
        """Background task to write queued data to file"""
    ...
    def Record_PelletData(self, MOUSEID, test_number, base_path):
        ...
        try:
            ...
            while not THRESHOLD_EXCEEDED.is_set():
                if ser.in_waiting > 0:
...
async def present_pellet(self, CurrentMouseID, NUM_TRIALS_PER_MOUSE, periphals_instance, Trail_Number, PIEZO_DIR):
    ...
    while periphals_instance.check_ir_breaker() != False:
        ...
        while not THRESHOLD_EXCEEDED.is_set():
            ...
            Camera_thread = threading.Thread(target=recorder.record_video())
            Camera_thread.start()
            ...
            Pizeo_thread = threading.Thread(target=PiezoRecorder_.Record_PelletData,
                                            args=(CurrentMouseID, Trail_Number, PIEZO_DIR))
            Pizeo_thread.start()
            ...
            # Signal all threads to stop (you'll need to modify your threads to check this flag)
            Interrupt_ = True
            # If you want to reset the flag after handling it:
            THRESHOLD_EXCEEDED.clear()
            for thread in threads:
                thread.join()
                print("Thread completed, continuing with execution")

        print("Now waiting for background thread to complete...")
        for thread in threads:
            thread.join()
            print("Thread completed, continuing with execution")
    ...
    while not Interrupt_:
        ...
        try:
            ...
            while trials_completed_today < NUM_TRIALS_PER_MOUSE:
                ...
                if (present_pellet(SingleTrackedData.normal_id, NUM_TRIALS_PER_MOUSE, ProfileID, periphals_instance,
                                   trials_completed_today, PIEZO_DIR)):
                    ...
                    new_trail_element = Trail_Element(trials_completed_today, trial_duration, completed, interrupted,
                                                      RFID_TAG_RAW)
                ...
                for trail_element in Trail_Elements:
                    ...
                    SingleTrackedData.add_trail_element(
                        Trail_Element(trials_completed_today, trial_duration, completed, interrupted, RFID_TAG_RAW))
...
def run_system(tracker):
    ...
    while True:
        if periphals_instance.check_ir_breaker() != False:
            ...
            global MOUSE_DIR
            MOUSE_DIR = os.path.join(HOME_DIR,
                                     str(Current_Mouse.normal_id) + " - " + str(Current_Mouse.raw_id) + "/" + str(
            ...
            tracker.trial_log_file = os.path.join(LOG_DIR,
                                                  str(datetime.now().strftime("%d-%m-%Y")) + "_trial_logs_.csv")
            tracker._ensure_trial_log_exists()
            if Current_Mouse.last_seen == str(datetime.now().strftime("%d-%m-%Y")):
                while Interrupt_ != False:
                    if Current_Mouse.trial_count < NUM_TRIALS_PER_MOUSE and Current_Mouse.test_time < MAX_DAILY_TIME_MIN:
                        ...
                        if trials_completed >= 0:
                            tracker.update_tracking(RFID_TAG_RAW, Current_Mouse.trail_num_total,
                                                    Current_Mouse.total_trail_time)
                ...
                while Interrupt_ != False:
                    ...
                    print(f"\n\nMouse ID {mouse_id} has Left the cage")
                    time.sleep(5)
                    break
